...
contract RonxMatrix is RonxBasic {
    ...
    function registration(
        ...
        // Deduct 25 BUSD and transaction fee from the sender's account
        uint totalAmount = (BASIC_PRICE * 2)+5 ;
        ...
        // Record transaction
        transactionHistory[userAddress].push(Transaction({
            amount: totalAmount,
    ...
    function generateReferralLink(address userAddress) private view returns (string memory) {
   ...
   function uintToString(uint v) private pure returns (string memory str) {
    ...
    // New function to get the transaction history of a user
    function getTransactionHistory(address userAddress) public view returns (Transaction[] memory) {
        return transactionHistory[userAddress];
    }

    // New function to calculate total revenue generated
    function getTotalRevenue() public view returns (uint) {
        uint totalRevenue = 0;
        for (uint i = 1; i < lastUserId; i++) {
            address userAddress = userIds[i];
            for (uint j = 0; j < transactionHistory[userAddress].length; j++) {
                totalRevenue += transactionHistory[userAddress][j].amount;
            }
        }
        return totalRevenue;
    }
